...
{
    public static Quaternion FaceFlatDir(Vector3 dir, float? pitch = null)
    {
        var flatDir = dir.Flat();
        if (flatDir.IsZero())
            return Quaternion.FromToRotation(Vector3.forward, Vector3.up);

        var flatRot = Quaternion.FromToRotation(Vector3.forward, flatDir);
        if (pitch is not { } yRot)
            return flatRot;

        var left = Vector3.Cross(flatDir, Vector3.up);
        var result = Quaternion.AngleAxis(yRot, left) * flatRot;
        return result;
    }
