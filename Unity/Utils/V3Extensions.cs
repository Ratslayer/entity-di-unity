...
public static class V3Extensions
{
    ...
    public static float FlatAngle(this Vector3 v1, Vector3 v2)
        => Mathf.Abs(SignedFlatAngle(v1, v2));

    public static float SignedFlatAngle(this Vector3 v1, Vector3 v2)
        => Vector3.SignedAngle(v1, v2, Vector3.up);
    ...
    public static Vector3 GetClosestAxisVector(this Vector3 dir)
    {
        var bestAxis = _axes[0];
        var bestDot = Vector3.Dot(dir, bestAxis);

        for (var i = 1; i < _axes.Length; i++)
        {
            var dot = Vector3.Dot(dir, _axes[i]);
            if (dot > bestDot)
            {
                bestDot = dot;
                bestAxis = _axes[i];
