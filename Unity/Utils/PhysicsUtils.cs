...
namespace BB
{
    public static class PhysicsUtils
    {
        ...
        readonly static Collider[] _colliderHits = new Collider[30];
        ...
        {
            var numHits = Physics.RaycastNonAlloc(origin, args._direction, _raycastHits, args._distance,
                args._layerMask, args._triggerInteraction);
            hits = _raycastHits;
            return numHits;
        }
        ...
        public static bool CapsuleCast(Vector3 bottom, float height, float radius, float skinWidth, in RaycastArgs args,
            out RaycastHit hit)
        {
        ...
        public static IReadOnlyCollection<T> SphereOverlapAll<T>(
            Vector3 point,
            float radius,
            in OverlapArgs args)
            where T : Component
        {
            ...
            return GetColliders<T>(numColliders);
        }

        static IReadOnlyCollection<T> GetColliders<T>(int numHits)
        {
            if (numHits == 0)
                return Array.Empty<T>();

            var result = new List<T>();
            foreach (var i in numHits)
                if (_colliderHits[i].TryGetComponent(out T comp))
                    result.Add(comp);
            return result;
        }
    }
